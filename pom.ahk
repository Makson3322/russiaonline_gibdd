...
CopyToClip(strText, notifyMsg) {
    Clipboard := strText
    SoundPlay, *-1
    TrayTip, ДПС ГИБДД [V4.3], %notifyMsg%, 2, 1
}
...
ShowSettingsGui(savedKey) {
    ...
    Gui, Settings:Add, Text, x0 y18 w%wW% Center, ПАМЯТКА
    Gui, Settings:Font, s9 c%CLR_ACCENT2% Bold, Segoe UI
    Gui, Settings:Add, Text, x0 y48 w%wW% Center, КУТУЗОВСКИЙ  •  РОССИЯ ОНЛАЙН  •  V4.3
...
SaveAndStart:
    ...
    TrayTip, ДПС ГИБДД V4.3, Настройки сохранены!`nКлавиша: [%CurrentHotkey%] | Тема: %CurrentTheme%, 3, 1
    BuildOverlay()
    BuildSelectorGui()
    BuildTimerGui()
    if (UpdateAvailable) {
...
BuildSelectorGui() {
    ...
    Gui, Selector:Add, Text, x0 y48 w%wW% Center, БАЗА ДАННЫХ ЗАКОНОДАТЕЛЬСТВА  •  V4.3
...
BuildOverlay() {
    ...
    Gui, Overlay:Add, Text, x390 y24 w70, [ V4.3 ]
    ...
    Gui, Overlay:Add, Button, x599 y98 w180 h30 gTabFZGIBDD, ФЗ о ГИБДД
...
FilterArticles:
    ...
    for idx, item in ArticleDB
    {
        ...
        if (catIndex == 1) {
            ...
            if (InStr(item.Category, "ФЗ ГИБДД"))
                passCategory := true
        }
        if (passCategory) {
...
УК РО|Статья 16.14 (Ф/С) Уклонение от следствия, задержания и суда|до 50 месяцев с созданием записи о судимости (5 звезд)|0
    ...
    fzAdditions1 =
    (
ФЗ ГИБДД|Статья 1. Государственная инспекция безопасности дорожного движения|ГИБДД — специализированное подразделение полиции в системе МВД РО, осуществляющее надзор в области безопасности дорожного движения|1|1. Статус^ГИБДД — специализированное подразделение полиции в системе МВД РО~2. Действие^На всей территории РО в пределах полномочий по ФЗ, ФЗ О полиции, КоАП РО, ПК РО~3. Самостоятельность^Обладает самостоятельной служебной компетенцией и не подменяет иные подразделения
ФЗ ГИБДД|Статья 2. Правовая основа деятельности ГИБДД|Конституция РО, федеральные кодексы, ФЗ О полиции, настоящий ФЗ и иные федеральные законы|0|1. Состав^Конституция РО, федеральные конституционные законы, кодексы, ФЗ О полиции, настоящий ФЗ~2. Акты меньшей силы^Применяются в части, не противоречащей актам большей юридической силы~3. Запрет внутренних актов^Внутренний акт не может создавать состав правонарушения, наказание, основание остановки или ограничение прав
ФЗ ГИБДД|Статья 3. Основные задачи ГИБДД|Обеспечение безопасности дорожного движения, предупреждение и пресечение правонарушений, надзор, регистрация ТС, выдача В/У|0|1. Обеспечение безопасности^Обеспечение безопасности дорожного движения~2. Предупреждение и пресечение^Предупреждение, выявление и пресечение правонарушений в области дорожного движения~3. Регулирование^Регулирование дорожного движения и устранение угроз~4. Производство по делам^Производство по делам об административных правонарушениях в пределах компетенции~5. Оформление ДТП^Оформление дорожно-транспортных происшествий~6. Выявление нарушителей^Выявление водителей без права управления либо в состоянии опьянения~7. Регистрация ТС^Государственная регистрация транспортных средств и ведение учета~8. Выдача В/У^Организация выдачи водительских удостоверений~9. Технический контроль^Контроль технического состояния ТС в предусмотренных законом случаях~10. Контроль сигналов^Контроль законности использования специальных световых и звуковых сигналов~11. Розыск^Участие в розыске транспортных средств и лиц~12. Колонны и мероприятия^Обеспечение безопасного движения колонн и государственных мероприятий
ФЗ ГИБДД|Статья 4. Принципы деятельности ГИБДД|Законность, уважение прав, необходимость и соразмерность, беспристрастность, приоритет жизни и здоровья, персональная ответственность|0
ФЗ ГИБДД|Статья 5. Соотношение с Федеральным законом «О полиции»|Сотрудник ГИБДД — сотрудник полиции; специальные дорожные полномочия дополняют общие полномочия полиции|0|1. Статус^Сотрудник ГИБДД является сотрудником полиции~2. Общие правила^Требования, задержание, обыск, применение силы — по ФЗ О полиции и ПК РО~3. Дополнение^Настоящий ФЗ устанавливает специальные дорожные полномочия
ФЗ ГИБДД|Статья 6. Правила дорожного движения|Единые обязательные ПДД утверждаются Правительством РО; ответственность за нарушение — по КоАП РО|0|1. Утверждение^Единые ПДД утверждаются Правительством РО~2. Содержание^Обязанности участников, сигналы, знаки, разметка, маневрирование, остановка, стоянка, световые приборы~3. Ответственность^Административная ответственность устанавливается КоАП РО, а не ПДД или внутренними актами
ФЗ ГИБДД|Статья 7. Территориальность и непрерывность дорожного надзора|Надзор на дорогах, улицах, площадях, парковках; территориальное распределение не препятствует пресечению очевидного нарушения|0|1. Территория надзора^Автомобильные дороги, улицы, площади, парковки общего пользования и иные территории~2. Непрерывность^Территориальное распределение не препятствует пресечению очевидного нарушения~3. Передача материала^После неотложных действий материал передается компетентному подразделению
ФЗ ГИБДД|Статья 8. Пределы вмешательства в дорожное движение|Любая мера должна иметь конкретное законное основание и применяться только в необходимом объеме|0|1. Законное основание^Требование, остановка, ограничение, досмотр, эвакуация — только по конкретному законному основанию~2. Запрет унижения^Остановка не может использоваться для унижения, задержки или личного конфликта
ФЗ ГИБДД|Статья 9. Система ГИБДД|Центральное руководство, территориальные, дорожно-патрульные, регистрационно-экзаменационные и технические подразделения|0|1. Состав системы^Центральное руководство, территориальные, дорожно-патрульные, регистрационно-экзаменационные подразделения~2. Внутренняя структура^Определяется Начальником ГИБДД~3. Наименование^Не создает дополнительных властных полномочий
ФЗ ГИБДД|Статья 10. Начальник ГИБДД|Назначается и освобождается Федеральным Правительством; срок полномочий — 30 календарных дней|0|1. Возглавляет ГИБДД^Начальник Государственной инспекции безопасности дорожного движения РО~2. Назначение^Федеральным Правительством~3. Срок^30 календарных дней; допускается повторное назначение~4. Звание^Определяется ФЗ О специальных и воинских званиях
ФЗ ГИБДД|Статья 11. Подчиненность ГИБДД|Общее руководство — Министр внутренних дел; оперативное — Начальник ГИБДД|0|1. Общее руководство^Министр внутренних дел РО~2. Оперативное руководство^Начальник ГИБДД~3. Взаимодействие^Руководитель ГУ МВД не вправе отменять решения Начальника ГИБДД по специальной дорожной компетенции
ФЗ ГИБДД|Статья 12. Полномочия Начальника ГИБДД|Организация деятельности, структура, назначения, планы надзора, приказы, контроль, взаимодействие, ограничения движения|0|1. Организация^Организует деятельность ГИБДД~2. Структура^Определяет внутреннюю структуру и распределяет компетенцию~3. Кадры^Назначает и освобождает сотрудников в пределах полномочий~4. Планы^Утверждает планы дорожного надзора и специальных мероприятий~5. Приказы^Издает обязательные для сотрудников приказы и инструкции~6. Контроль^Осуществляет ведомственный контроль~7. Регистрация^Организует регистрационно-экзаменационную деятельность~8. Взаимодействие^Обеспечивает взаимодействие с МВД, прокуратурой, СК, ФСБ, судами, ВС, службами~9. Ограничения движения^Принимает решения о длительных или масштабных ограничениях~10. Иные полномочия^Установленные федеральным законодательством
    )
    LoadData(fzAdditions1)

    fzAdditions2 =
    (
ФЗ ГИБДД|Статья 13. Заместитель Начальника ГИБДД|Назначается и освобождается Начальником ГИБДД; руководит порученными направлениями|0|1. Назначение^Начальником ГИБДД, если иной порядок не установлен~2. Полномочия^Руководит порученными направлениями и замещает Начальника~3. ВРИО^Не считается самостоятельным сроком полномочий для высшего звания
ФЗ ГИБДД|Статья 14. Сотрудники ГИБДД и специальные звания|Проходят службу в полиции; предельные звания, порядок присвоения — по ФЗ О званиях|0|1. Служба^Сотрудники проходят службу в полиции и имеют специальные звания~2. Звания^Определяются ФЗ О специальных и воинских званиях~3. Перевод^С сохранением звания с учетом предельного звания новой должности
ФЗ ГИБДД|Статья 15. Служебное удостоверение, нагрудный знак и идентификация сотрудника|Обязан иметь удостоверение и служебный номер; представиться и сообщить причину обращения|0|1. Удостоверение^Обязан иметь служебное удостоверение и индивидуальный служебный номер~2. Представление^Обязан представиться и по требованию сообщить причину обращения или остановки~3. Идентификация^При существенном ограничении прав сотрудник обязан быть идентифицируем
ФЗ ГИБДД|Статья 16. Внутренние акты ГИБДД|Приказы, инструкции и регламенты по патрулированию, службе, оформлению, учету, экзаменам|0|1. Право издавать^Начальник ГИБДД издает приказы, инструкции и регламенты~2. Пределы^Внутренний акт не может расширять основания остановки, обыска, задержания или ответственности
ФЗ ГИБДД|Статья 17. Дорожно-патрульная деятельность|Патрулирование дорог, наблюдение, реагирование, регулирование; частное ТС для патруля не допускается|0|1. Содержание^Патрулирование, наблюдение, реагирование на сообщения, регулирование движения~2. Способы^Служебные ТС, пешие наряды, стационарные посты и иные законные способы~3. Частное ТС^Не допускается для обычной службы, кроме неотложного случая
ФЗ ГИБДД|Статья 18. Право сотрудника ГИБДД требовать остановку транспортного средства|Требование об остановке должно быть однозначным и ясно воспринимаемым водителем|0|1. Право^Вправе заявить обязательное требование об остановке при наличии основания~2. Форма^Жест, специальное устройство, громкоговоритель, световой сигнал или иной ясно воспринимаемый способ~3. Остановка^В ближайшем месте, где остановка безопасна
ФЗ ГИБДД|Статья 19. Основания остановки транспортного средства|10 законных оснований, включая признаки правонарушения, проверку документов, ориентировку, неисправность, опьянение, спецмероприятие, регулирование, ДТП, угрозу жизни|1|1. Признаки нарушения^Непосредственное выявление либо техническая фиксация признаков дорожного правонарушения~2. Проверка документов^Проверка права управления, регистрационных документов и знаков~3. Ориентировка или розыск^Сведения о причастности ТС или лица к правонарушению~4. Неисправность^Внешние признаки технической неисправности, создающей угрозу~5. Опьянение или состояние^Признаки опьянения, заболевания или иного состояния, исключающего безопасное управление~6. Контрольный пост^Законно установленный контрольный пост, заградительное мероприятие или спецоперация~7. Регулирование^Регулирование движения, пропуск спецтранспорта, колонны, охраняемого мероприятия~8. ДТП^ДТП или основания полагать, что ТС связано с ДТП~9. Угроза жизни^Предотвращение непосредственной угрозы жизни и здоровью~10. Иные случаи^Прямо предусмотренные федеральным законом
ФЗ ГИБДД|Статья 20. Порядок остановки транспортного средства|Сообщить причину остановки без промедления; проверка не дольше объективно необходимого времени|0|1. Сообщение причины^Без необоснованного промедления сообщает причину остановки и предъявляет требования~2. Длительность^Не дольше времени, объективно необходимого для проверки и разрешения материала~3. Прекращение^Если основание не подтвердилось и нет другого — дальнейшее удержание прекращается
ФЗ ГИБДД|Статья 21. Требование водителю или пассажиру покинуть транспортное средство|7 оснований для требования покинуть ТС; отказ сам по себе не дает основания для смертельной силы|0|1. Отстранение^Отстранить водителя от управления~2. Освидетельствование^Провести законное освидетельствование~3. Задержание или личность^Задержать или установить личность лица~4. Осмотр или обыск^Произвести предусмотренный законом осмотр, досмотр или обыск~5. Идентификация или техсостояние^Проверить идентификационные обозначения или техническое состояние~6. Угроза безопасности^Устранить непосредственную угрозу безопасности~7. Процессуальный акт^Исполнить иной процессуальный акт~Пассажир^Такое требование возможно при задержании, обеспечении безопасности или процессуальном действии~Запрет смертельной силы^Отказ выйти сам по себе не является основанием для смертельной силы
ФЗ ГИБДД|Статья 22. Проверка документов водителя и транспортного средства|Проверка В/У, регистрационных документов и иных обязательных документов по базам|0|1. Документы^В/У, регистрационные документы и иные документы, обязанность иметь которые установлена законом~2. Проверка по базам^Документы могут быть проверены по государственным информационным системам~3. Удержание^Не вправе удерживать документ после проверки без законного основания
ФЗ ГИБДД|Статья 23. Проверка государственных регистрационных знаков и идентификационных обозначений|Проверка знаков по учету и идентификационных обозначений; без превращения в полный обыск|0|1. Проверка знаков^Соответствие знака сведениям учета~2. Идентификационные обозначения^Проверка при необходимости подтвердить регистрацию или признаки подделки~3. Пределы^Проверка не должна превращаться в полный обыск без самостоятельного основания~4. Подложный знак^Меры по КоАП РО и ПК РО
    )
    LoadData(fzAdditions2)

    fzAdditions3 =
    (
ФЗ ГИБДД|Статья 24. Осмотр и обыск транспортного средства|Визуальный осмотр допустим; полный обыск — только по ПК РО|0|1. Визуальный осмотр^Внешнее состояние, знаки, открыто видимые предметы и груз~2. Обыск^Открытие отсеков и обследование личных вещей — только по ПК РО~3. Запрет автоматического обыска^Отсутствие знака, нарушение стоянки или превышение скорости не дают основания для полного обыска
ФЗ ГИБДД|Статья 25. Использование технических средств дорожного надзора|Фото- видеофиксация, измерение скорости, проверка знаков; учет технической ошибки|0|1. Средства^Фото- видеофиксация, измерение скорости, проверка знаков, техническое состояние и иные законные средства~2. Доказательство^Результат используется при возможности установить относимость, время и обстоятельства~3. Техническая ошибка^Сотрудник обязан учитывать очевидную ошибку
ФЗ ГИБДД|Статья 26. Регулирование дорожного движения|Ручное регулирование, изменение очередности, запрет или разрешение движения; сигналы регулировщика приоритетны|0|1. Право регулирования^Ручное регулирование, изменение очередности, запрет или разрешение движения~2. Приоритет сигналов^Сигналы регулировщика приоритетны перед светофором, знаком и разметкой~3. Прекращение^Ограничение прекращается после отпадения основания
ФЗ ГИБДД|Статья 27. Временное ограничение или прекращение движения|При ДТП, пожаре, аварии, опасном предмете, массовых беспорядках, розыске, мероприятии|0|1. Основания^ДТП, пожар, авария, повреждение дороги, опасный предмет, беспорядки, процессуальное действие, розыск, мероприятие~2. Соразмерность^Территориально и по времени соразмерно цели~3. Длительное перекрытие^По решению Начальника ГИБДД, заместителя или иного уполномоченного руководителя
ФЗ ГИБДД|Статья 28. Административная компетенция ГИБДД|Возбуждает, рассматривает и разрешает дела по КоАП РО в пределах компетенции|0|1. Компетенция^Возбуждает, рассматривает и разрешает дела, отнесенные КоАП РО к компетенции ГИБДД~2. Статья 10 КоАП^Рассматривает материалы по статье 10 КоАП РО в пределах подведомственности~3. Исключительная компетенция^Пресекает, сохраняет доказательства и передает материал суду или прокуратуре
ФЗ ГИБДД|Статья 29. Упрощенное рассмотрение дорожного правонарушения на месте|На месте при достаточных доказательствах, с объявлением лицу существа, статьи и права обжалования|0|1. Условия^Дорожное правонарушение и достаточные доказательства~2. Объявление^Существо, статья, доказательства, мера, право обжалования~3. Отдельное дело^Не требуется, когда Кодекс допускает упрощенное рассмотрение
ФЗ ГИБДД|Статья 30. Составление и передача административного материала|Оформление и передача компетентному органу; сохранение доказательств и мер обеспечения|0|1. Оформление^Если материал требует судебного или прокурорского разрешения либо не может быть разрешен на месте~2. Сохранение^Передача не прекращает обязанность обеспечить сохранность доказательств и мер обеспечения
ФЗ ГИБДД|Статья 31. Лишение права управления транспортными средствами|Сотрудник ГИБДД не вправе самостоятельно назначать лишение права; оформляет материал в суд|0|1. Запрет самостоятельного назначения^Относится к компетенции суда по КоАП РО~2. Направление в суд^Материал направляется мировому судье или в иной компетентный суд~3. Исполнение^ГИБДД исполняет вступивший в силу судебный акт, вносит сведения в учет
ФЗ ГИБДД|Статья 32. Ведомственный пересмотр решения сотрудника ГИБДД|Пересмотр вышестоящим должностным лицом по ст. 9.5 КоАП РО|0|1. Право^Лицо вправе потребовать пересмотра решения вышестоящим должностным лицом ГИБДД~2. Порядок^По статье 9.5 КоАП РО~3. Материалы^Сотрудник обязан предоставить все доказательства в установленный срок
ФЗ ГИБДД|Статья 33. Обязательная фиксация административных действий|Фиксация ареста, обыска, освидетельствования, изъятия, применения силы и иных обязательных действий|0|1. Фиксация^Арест, принудительный обыск, освидетельствование, принудительное изъятие, применение силы~2. Сохранение материалов^В объеме и срок, позволяющие пересмотр
ФЗ ГИБДД|Статья 34. Административная ответственность государственного служащего|Вне службы — общий порядок; при связи со службой или аресте — прокурорский порядок|0|1. Вне службы^Рассматривается на общих основаниях~2. Связь со службой или арест^Применяется специальный прокурорский порядок
ФЗ ГИБДД|Статья 35. Отстранение от управления транспортным средством|6 оснований, включая отсутствие права, опьянение, состояние, угрозу, задержание, лишение права|0|1. Нет права управления^Не имеет права управления соответствующей категорией~2. Опьянение^Находится в состоянии опьянения~3. Состояние^Физическое или психическое состояние, исключающее безопасное управление~4. Угроза^Дальнейшая эксплуатация представляет непосредственную угрозу~5. Задержание^Подлежит задержанию~6. Лишен права^Лишено права управления действующим судебным актом~Прекращение^Действует до отпадения основания
ФЗ ГИБДД|Статья 36. Основания освидетельствования на состояние опьянения|Конкретные признаки опьянения либо иные достаточные основания|0|1. Допустимость^При наличии конкретных признаков возможного опьянения либо достаточных оснований~2. Признаки^Запах алкоголя, нарушение речи или координации, неадекватное поведение, признаки наркотического опьянения, опасное управление, обстоятельства ДТП~3. Запрет^Не проводится исключительно из любопытства или как наказание
ФЗ ГИБДД|Статья 37. Порядок освидетельствования|Уполномоченный сотрудник или медик; видеофиксация; право на дополнительное медицинское освидетельствование|0|1. Проведение^Уполномоченным сотрудником с допущенным средством либо медиком~2. Объявление^Основание освидетельствования и последствия отказа~3. Видеофиксация^Существенный ход и результат подлежат фиксации~4. Дополнительное освидетельствование^При споре, несогласии или невозможности проверки на месте — право требовать медицинского освидетельствования
    )
    LoadData(fzAdditions3)

    fzAdditions4 =
    (
ФЗ ГИБДД|Статья 38. Отказ от освидетельствования|Ясно выраженный отказ после разъяснения; физическая невозможность не считается отказом|0|1. Понятие отказа^Ясно выраженный отказ после разъяснения основания и последствий~2. Физическая невозможность^Не признается отказом, если лицо предлагает иной законный способ~3. Материал^Оформляется и направляется в суд, если санкция предусматривает лишение права управления
ФЗ ГИБДД|Статья 39. Последствия выявления состояния опьянения|Отстранение от управления; задержание ТС или передача трезвому лицу; материал в суд|0|1. Отстранение^Водитель с достаточными доказательствами опьянения отстраняется от управления~2. ТС^Задерживается или передается иному трезвому лицу с правом управления~3. Материал^Разрешается или передается в суд по КоАП РО
ФЗ ГИБДД|Статья 40. Употребление вызывающих опьянение веществ после ДТП или остановки|Запрет употребления алкоголя и веществ до завершения освидетельствования|0
ФЗ ГИБДД|Статья 41. Задержание транспортного средства|Задержание — мера обеспечения, не наказание; основание объявляется и фиксируется|0|1. Основания^В случаях, установленных КоАП РО~2. Не наказание^Не должна использоваться как дополнительное не предусмотренное законом наказание~3. Объявление^Основание объявляется водителю или владельцу и фиксируется в материалах
ФЗ ГИБДД|Статья 42. Перемещение транспортного средства на специализированное место хранения|Перемещение при угрозе безопасности, препятствии движению, незаконной эксплуатации|0|1. Допустимость^При угрозе безопасности, препятствии движению, незаконной эксплуатации или ином законном случае~2. Немедленное устранение^Если основание устранимо и дальнейшее управление законно — перемещение не производится~3. Сохранность^Принимаются разумные меры к сохранности ТС и имущества
ФЗ ГИБДД|Статья 43. Возврат задержанного транспортного средства|Возврат после отпадения основания; отказ может быть обжалован|0|1. Возврат^После отпадения основания задержания~2. Исключения^Если ТС удерживается как вещественное доказательство или предмет конфискации~3. Обжалование^Отказ может быть обжалован вышестоящему лицу, прокурору или в суд
ФЗ ГИБДД|Статья 44. Государственная регистрация транспортных средств|Регистрация, учет, выдача документов и знаков; отказ может быть обжалован|0|1. Регистрация^ГИБДД осуществляет регистрацию, ведет учет, выдает документы и знаки~2. Документы^Подтверждение права собственности, идентификационные сведения и иные данные~3. Отказ^При отсутствии оснований, признаках подделки или в иных случаях~4. Обжалование^Отказ обжалуется вышестоящему лицу ГИБДД либо в суд
ФЗ ГИБДД|Статья 45. Государственные регистрационные знаки|Присваиваются при регистрации; подложный или измененный знак влечет последствия|0|1. Присвоение^При государственной регистрации~2. Запрет^Использование знака другого ТС, подложного или измененного~3. Изъятие^ГИБДД вправе изъять заведомо подложный знак как предмет материала или доказательство
ФЗ ГИБДД|Статья 46. Выдача водительских удостоверений|Проверка права на управление, выдача В/У, порядок экзамена; отказ мотивирован|0|1. Организация^Проверка права на управление и выдача В/У~2. Первоначальная выдача^После подтверждения личности, отсутствия запрета и успешной проверки знаний и навыков~3. Порядок^Экзамен, категории и форма В/У — актом Правительства или МВД~4. Отказ^Мотивирован и может быть обжалован
ФЗ ГИБДД|Статья 47. Исполнение лишения права управления и восстановление права|Учет лишенных права; исполнение судебных актов; восстановление после срока|0|1. Учет^Ведет учет лиц, лишенных права управления~2. Исполнение^Обеспечивает исполнение вступивших в силу судебных актов~3. Восстановление^После окончания срока при отсутствии другого запрета~4. Запрет продления^Не вправе продлевать срок лишения внутренним решением
ФЗ ГИБДД|Статья 48. Контроль технического состояния транспортного средства|Проверка внешне выявляемого состояния; при угрозе — запрет движения и задержание|0|1. Проверка^Внешне выявляемое техническое состояние при признаках неисправности~2. Угроза^При непосредственной угрозе — запрет движения и задержание ТС~3. Незначительная неисправность^Сама по себе не должна влечь задержание, если закон не устанавливает иное
ФЗ ГИБДД|Статья 49. Изменение конструкции транспортного средства|Существенное изменение конструкции — в порядке ФЗ; ГИБДД проверяет соответствие|0|1. Изменение^Допускается в порядке, установленном ФЗ~2. Проверка^ГИБДД вправе проверять соответствие изменений требованиям и регистрационным сведениям~3. Внутренний акт^Не может запрещать изменение, разрешенное актом большей силы
ФЗ ГИБДД|Статья 50. Специальные световые и звуковые сигналы|Учет и контроль законности установки и использования; изъятие незаконных устройств|0|1. Учет и контроль^ГИБДД осуществляет учет и контроль законности установки и использования~2. Право использования^Только в случаях и порядке, установленных ФЗ~3. Изъятие^При выявлении незаконного устройства — меры к изъятию в порядке производства
ФЗ ГИБДД|Статья 51. Реагирование на дорожно-транспортное происшествие|Прибытие, безопасность, организация движения, фиксация; приоритет — помощь пострадавшим|0|1. Реагирование^Принимает меры к прибытию, обеспечению безопасности, организации движения и фиксации~2. Пострадавшие^Приоритет — вызов медиков, первая помощь, устранение угрозы~3. Ограничение движения^Организует временное ограничение и охрану места
    )
    LoadData(fzAdditions4)

    fzAdditions5 =
    (
ФЗ ГИБДД|Статья 52. Обязанности сотрудника ГИБДД на месте ДТП|9 обязанностей, включая участников, документы, освидетельствование, фиксацию, объяснения, доказательства|0|1. Участники^Установить участников и очевидцев~2. Документы и состояние^Проверить документы и состояние водителей~3. Освидетельствование^Организовать при наличии оснований~4. Фиксация^Фото- видеофиксация, измерения, схема происшествия~5. Повреждения и следы^Зафиксировать повреждения и существенные следы~6. Объяснения^Получить объяснения~7. Условия^Установить знаки, разметку, сигналы и видимость~8. Признаки правонарушения^Выявить признаки административного правонарушения или преступления~9. Сохранность^Обеспечить сохранность доказательств
ФЗ ГИБДД|Статья 53. Освобождение проезжей части после ДТП|После достаточной фиксации; без причины обстановку не менять; при угрозе — до фиксации|0|1. Освобождение^После достаточной фиксации положения, повреждений и следов~2. Запрет изменения^До необходимой фиксации без причины обстановку не менять~3. Угроза^При непосредственной угрозе — перемещение до полной фиксации
ФЗ ГИБДД|Статья 54. ДТП только с имущественным ущербом|При отсутствии пострадавших и согласии участников — самостоятельное оформление после фиксации|0|1. Самостоятельное оформление^Без пострадавших и при согласии сторон — после фото- или видеофиксации~2. Вызов ГИБДД^При споре, опьянении, оставлении места, отсутствии права управления и иных нарушениях~3. Соглашение^Не препятствует привлечению за самостоятельно выявленное нарушение
ФЗ ГИБДД|Статья 55. Определение административно значимых обстоятельств ДТП|ГИБДД устанавливает фактические обстоятельства и причинную связь; гражданско-правовое возмещение — отдельно|0|1. Обстоятельства^Устанавливает фактические обстоятельства для решения об ответственности~2. Гражданско-правовое возмещение^Решение ГИБДД не определяет окончательный размер, если не урегулирован добровольно или судом
ФЗ ГИБДД|Статья 56. ДТП с признаками преступления|Прекращение самостоятельного разрешения и передача по подследственности|0|1. Передача^Прекращает самостоятельное разрешение и передает компетентному органу~2. Неотложные действия^Спасение людей, задержание, сохранение доказательств~3. Административный эпизод^Отдельный эпизод может разрешаться самостоятельно, если допускается законом
ФЗ ГИБДД|Статья 57. Материалы ДТП и их предоставление|Материалы должны позволять проверить обстоятельства; доступ участников, адвокатов, суда, прокуратуры|0|1. Материалы^Позволяют проверить обстоятельства и происхождение доказательств~2. Доступ^Участники, адвокаты, суд, прокуратура, следствие и иные уполномоченные лица
ФЗ ГИБДД|Статья 58. Преследование транспортного средства|При неисполнении требования об остановке, при задержании, опасности; соразмерность риска|0|1. Начало^При неисполнении требования об остановке, задержании, опасности или ином основании~2. Обозначение^Повторяет требование и обозначает принадлежность к полиции~3. Соразмерность^При превышении риска — изменить тактику или прекратить преследование
ФЗ ГИБДД|Статья 59. Переход к принудительной остановке|После неисполнения требования; повторное предупреждение не всегда требуется; неостановка сама по себе не дает права на смертельную силу|0|1. Допустимость^После неисполнения требования при существенной опасности или задержании~2. Без предупреждения^Если ТС используется как средство нападения или создает непосредственную угрозу~3. Смертельная сила^Сам факт неостановки не разрешает смертельную силу
ФЗ ГИБДД|Статья 60. Средства принудительной остановки|Служебные ТС, заграждения, устройства принудительной остановки; оружие — только при самостоятельных основаниях|0|1. Средства^Служебные ТС, дорожные заграждения, устройства принудительной остановки, изменение организации движения~2. Контакт^С учетом скорости, обстановки, пассажиров и риска для посторонних~3. Оружие^Только при самостоятельных основаниях по ФЗ О полиции; стрельба по колесам сама по себе не основание
ФЗ ГИБДД|Статья 61. Принудительное извлечение лица из транспортного средства|После законной остановки при основаниях статьи 21; сила соразмерна; после сопротивления — снижение|0|1. Требование^После законной остановки при основаниях статьи 21~2. Сила^При отказе и риске побега, сопротивления, уничтожения доказательств~3. Снижение^После прекращения сопротивления интенсивность немедленно снижается
ФЗ ГИБДД|Статья 62. Контрольные посты и заградительные мероприятия|Временные посты при розыске, мероприятии, ЧС, спецоперации; обыск — отдельное основание|0|1. Основания^Розыск, массовое мероприятие, ЧС, спецоперация, угроза безопасности~2. Проверка^Остановка ТС для проверки документов и сведений, относящихся к цели~3. Обыск^Требует самостоятельного процессуального основания
ФЗ ГИБДД|Статья 63. Организованные транспортные колонны и сопровождение|Сопровождение кортежей и колонн; временные ограничения движения|0|1. Сопровождение^Государственные кортежи, специальные перевозки, колонны~2. Ограничения^Вправе временно ограничивать движение и требовать не вклиниваться~3. Объем^Только в необходимом объеме
ФЗ ГИБДД|Статья 64. Преимущество транспортных средств со специальными сигналами|Преимущество при неотложном задании; безопасность; пресечение опасного использования|0|1. Преимущество^При законном использовании сигналов при неотложном задании~2. Безопасность^Водитель спецТС обязан принимать разумные меры~3. Пресечение^ГИБДД вправе пресечь явно опасное или неправомерное использование
ФЗ ГИБДД|Статья 65. Специальные профилактические мероприятия|По решению руководства; определенная цель; не расширяет полномочий|0|1. Решение^Начальник ГИБДД, заместитель или иной уполномоченный руководитель~2. Цель^Выявление опьянения, розыска, подложных знаков, неисправностей, грубых нарушений~3. Пределы^Не предоставляет полномочий, отсутствующих в ФЗ~4. Оформление^В обычном процессуальном порядке
    )
    LoadData(fzAdditions5)

    fzAdditions6 =
    (
ФЗ ГИБДД|Статья 66. Дорожные работы, опасные участки и обязательные предписания|Временные меры безопасности, предписание владельцу дороги, ограничение движения|0|1. Меры^Временные меры безопасности при опасном состоянии дороги~2. Предписание^Обязательное законное предписание владельцу дороги или ответственному органу~3. Ограничение движения^При непосредственной угрозе — до устранения опасности~4. Ответственность^Неисполнение предписания — по КоАП РО
ФЗ ГИБДД|Статья 67. Взаимодействие с иными подразделениями МВД РО|Взаимное содействие; специальный дорожный контроль — за ГИБДД|0|1. Содействие^Задержание лиц, розыск ТС, охрана места, массовые мероприятия~2. Обычная полиция^Вправе остановить ТС по ФЗ О полиции; специальный контроль — ГИБДД~3. Передача^ГИБДД вправе передать задержанное лицо сотруднику МВД с материалами
ФЗ ГИБДД|Статья 68. Взаимодействие со Следственным комитетом|Исполнение законных поручений СК в пределах компетенции|0|1. Поручения^Розыск ТС, установление маршрута, обеспечение остановки, охрана места ДТП~2. Пределы^Поручение не дает права принимать процессуальное решение исключительной компетенции
ФЗ ГИБДД|Статья 69. Взаимодействие с Федеральной службой безопасности|Содействие ФСБ при розыске, контртеррористическом мероприятии, охране маршрута|0
ФЗ ГИБДД|Статья 70. Взаимодействие с прокуратурой|Надзор прокуратуры; предоставление материалов; решение прокурора по госслужащему обязательно|0|1. Надзор^Прокуратура осуществляет надзор за соблюдением ФЗ~2. Предоставление материалов^ГИБДД обязана предоставлять истребованные материалы, записи и сведения~3. Не руководство^Прокурорский надзор не является повседневным административным руководством~4. Решение прокурора^По задержанию госслужащего обязательно для сотрудников ГИБДД
ФЗ ГИБДД|Статья 71. Взаимодействие с судами|Направление материалов обязательного судебного порядка; исполнение вступивших в силу актов|0|1. Направление^Материалы обязательного судебного порядка~2. Предоставление^Истребованные материалы для проверки законности решения~3. Запрет игнорирования^Сотрудник не вправе игнорировать или отменять судебный акт
ФЗ ГИБДД|Статья 72. Взаимодействие с медицинскими, спасательными и дорожными службами|Взаимодействие при ДТП, освидетельствовании, устранении препятствий|0|1. Медицинские и спасательные^При ДТП, освидетельствовании и угрозе жизни~2. Дорожные и коммунальные^При устранении препятствий и повреждений~3. Требование^Должно относиться к компетенции и конкретной законной цели
ФЗ ГИБДД|Статья 73. Взаимодействие с Вооруженными Силами и военными объектами|Сопровождение военных колонн; доступ на режимную территорию с учетом режима|0|1. Сопровождение^Военные колонны, перевозка вооружения, специальные перевозки~2. Доступ^С учетом режима, кроме неотложного пресечения угрозы или прямого акта
ФЗ ГИБДД|Статья 74. Ведомственный контроль|Проверка законности работы сотрудников, отмена незаконных решений, дисциплинарные меры|0|1. Права^Проверять законность и качество работы, истребовать материалы, отменять незаконные решения~2. Запрет^Не может использоваться для сокрытия доказательств, уничтожения жалобы или давления
ФЗ ГИБДД|Статья 75. Обжалование действий и решений ГИБДД|Жалоба вышестоящему лицу, в прокуратуру или в суд; ведомственный пересмотр — по КоАП РО|0|1. Жалоба^Вышестоящему должностному лицу, в прокуратуру или в суд~2. Ведомственный пересмотр^По правилам КоАП РО~3. Иная жалоба^По общим правилам процессуального законодательства~4. Приостановление^Подача жалобы сама по себе не приостанавливает меру
ФЗ ГИБДД|Статья 76. Ответственность сотрудника ГИБДД|Дисциплинарная, административная, гражданско-правовая и уголовная ответственность|0|1. Виды^Дисциплинарная, административная, гражданско-правовая и уголовная~2. Злоупотребления^Незаконная остановка, фальсификация, ложные сведения, незаконное лишение права, эвакуация, освидетельствование, сокрытие записи~3. Отмена решения^Сама по себе не доказывает вину сотрудника
ФЗ ГИБДД|Статья 77. Возмещение вреда|Вред, причиненный незаконным действием ГИБДД, возмещается по ФЗ|0
    )
    LoadData(fzAdditions6)
}
